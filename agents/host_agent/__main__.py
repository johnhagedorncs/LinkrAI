...
async def get_response_from_agent(
    ...
    try:
        ...
        async for event in event_iterator:
            # 🔎 Drop the entire event object every time
            print("=== RAW EVENT DEBUG START ===")
            print(f"Type: {type(event)}")
            try:
                # Pydantic objects: model_dump works nicely
                print(event.model_dump_json(exclude_none=True, indent=2))
            except Exception as e:
                print(f"(Could not dump event as JSON: {e})")
                print(event)
            print("=== RAW EVENT DEBUG END ===")
...
async def main():
    ...
    with gr.Blocks(
        ...
        gr.ChatInterface(
            ...
            description='This assistant can help you with billing issues, prescription issues, medical referrals, and appointment scheduling. It can execute complete workflows including referral creation followed by appointment booking.',
        )
